...
using CB.Application.Validators.OutOfCoinMoneySign;
...
namespace CB.Web.Controllers
{
    ...
    [Authorize]
    public class OutOfCoinMoneySignController : ControllerBase
    {
        private readonly IOutOfCoinMoneySignService _coinMoneySignService;
        private readonly OutOfCoinMoneySignCreateValidator _createValidator;
        private readonly OutOfCoinMoneySignEditValidator _editValidator;

        public OutOfCoinMoneySignController(
            IOutOfCoinMoneySignService coinMoneySignService,
            OutOfCoinMoneySignCreateValidator createValidator,
            OutOfCoinMoneySignEditValidator editValidator
        )
        {
        ...
        {
            List<OutOfCoinMoneySignGetDTO> data = await _coinMoneySignService.GetAllAsync();
            return Ok(data);
        }
        ...
        {
            OutOfCoinMoneySignGetDTO? data = await _coinMoneySignService.GetByIdAsync(id);
            if (data is null)
            {
        ...
        [HttpPost]
        public async Task<IActionResult> Create([FromForm] OutOfCoinMoneySignCreateDTO dTO)
        {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Edit(int id, [FromForm] OutOfCoinMoneySignEditDTO dTO)
        {
