...
namespace CB.Application.Validators.Faq
{
    public class FaqEditValidator : AbstractValidator<FaqEditDTO>
    {
        public FaqEditValidator(
            IFaqCategoryService faqCategoryService
        )
        {

            RuleFor(x => x.FaqCategoryId)
                    .MustAsync(async (faqCategoryID, cancellation) =>
                    {
                        var faqCategory = await faqCategoryService.GetByIdAsync(faqCategoryID);
                        return faqCategory != null ? true : false;
                    })
