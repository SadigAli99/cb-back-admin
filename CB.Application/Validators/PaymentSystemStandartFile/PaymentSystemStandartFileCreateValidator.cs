...
namespace CB.Application.Validators.PaymentSystemStandartFile
{
    public class PaymentSystemStandartFileCreateValidator : AbstractValidator<PaymentSystemStandartFileCreateDTO>
    {
        public PaymentSystemStandartFileCreateValidator(IPaymentSystemStandartService paymentSystemStandartService)
        {
            ...
            RuleFor(x => x.PaymentSystemStandartId)
                .NotNull().WithMessage("Kateqoriya daxil edin")
                .MustAsync(async (paymentSystemStandartFileId, cancellation) =>
                {
                    var paymentSystemStandartFile = await paymentSystemStandartService.GetByIdAsync(paymentSystemStandartFileId);
                    return paymentSystemStandartFile != null ? true : false;
                })
