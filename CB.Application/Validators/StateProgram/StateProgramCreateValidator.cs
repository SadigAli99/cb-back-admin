...
namespace CB.Application.Validators.StateProgram
{
    public class StateProgramCreateValidator : AbstractValidator<StateProgramCreateDTO>
    {
        public StateProgramCreateValidator(IStateProgramCategoryService participantCategoryService)
        {
            ...
            RuleFor(x => x.StateProgramCategoryId)
                .NotNull().WithMessage("Kateqoriya daxil edin")
                .MustAsync(async (participantCategoryId, cancellation) =>
                {
                    var participantCategory = await participantCategoryService.GetByIdAsync(participantCategoryId);
                    return participantCategory != null ? true : false;
                })
