...
namespace CB.Application.Validators.FinancialStabilityReport
{
    public class FinancialStabilityReportCreateValidator : AbstractValidator<FinancialStabilityReportCreateDTO>
    {
        public FinancialStabilityReportCreateValidator()
        {
            RuleFor(x => x.File)
            ...
            RuleFor(x => x.CoverTitles)
                .NotEmpty().WithMessage("Ən azı bir dil üçün fayl başlıq daxil edilməlidir.");

            RuleForEach(x => x.CoverTitles)
                .Must(v => v.Value != null && v.Value.Length <= 500)
                .WithMessage("Bu dil üçün fayl başlıq 500 simvoldan artıq ola bilməz.");
        }
    }
}
