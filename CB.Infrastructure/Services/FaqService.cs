...
namespace CB.Infrastructure.Services
{
    public class FaqService : IFaqService
    {
        ...
        public async Task<List<FaqGetDTO>> GetAllAsync()
        {
            ...
            List<FaqGetDTO> data = _mapper.Map<List<FaqGetDTO>>(entities);

            foreach (var item in data)
            {
                var entity = entities.FirstOrDefault(x => x.Id == item.Id);
                if (entity is null) continue;
                item.FaqCategory = entity.FaqCategory?.Translations.FirstOrDefault(x => x.LanguageId == 1)?.Title;
            }
            return data;
        }
        public async Task<FaqGetDTO?> GetByIdAsync(int id)
        {
            ...
            FaqGetDTO? data = _mapper.Map<FaqGetDTO>(entity);
            data.FaqCategory = entity.FaqCategory?.Translations.FirstOrDefault(x => x.LanguageId == 1)?.Title;
            return data;
        }
        ...
        public async Task<bool> UpdateAsync(int id, FaqEditDTO dto)
        {
            var entity = await _repository.GetQuery()
                        .Include(b => b.Translations)
                        .ThenInclude(bt => bt.Language)
